...
class AdmUserController extends BaseController
{
    ...
    public function addGradeAction()
    {
        ...
        //$section = SectionSvc::getAllSection();//根据id获取区块儿
        $adminuser = AdmUserSvc::getById($id);//当前管理员
        $haveauth = AdmAuthSvc::getAuthByUid($id);

        $auths = AdmAuthSvc::getAll();
        $str = '<form action="/AdmAuth/addauth/" method="post" id="authForm"><div class="baobiao">';
        $str.="<span style='color:red'>已有的权限不会显示在此列表！</span><br/><br/>";
        ...
        $str .= '<table width="100%" class="table01">';
        if(!empty($tempArr)){
            $tempsign = 'check';
            $count = 0;

            $tdName = $tempsign.(string)($count ++);
            $str .= ' <tr>
                         <td id="'.$tdName.'Td">';
            foreach($tempArr as $key=>$item){
                ...
                $str.= $sign."<span>【<input type='checkbox' name='auths[]' value='".$item['id']."'>".$item['name']."】</span>";
                if($key%5==0 && $key!=0){$str.="<br><hr>";}
